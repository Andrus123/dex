...
contract Exchange is ERC20 {
    ...
    // Exchange is inheriting ERC20, because our exchange would keep track of CryptoDev LP tokens
    constructor(address _CryptoDevToken) ERC20("CryptoDev LP Token", "CDLP") {
        require(_CryptoDevToken != address(0), "Token address passed is a null address");
        cryptoDevTokenAddress = _CryptoDevToken;
    }
    ///@dev Returns the amount of 'CryptoDevTokens' held by the contract
    ...
    function addLiquidity(uint _amount) public payable returns(uint) {
        ...
        Ether and CryptoDev tokens because there is no ratio currently
         */
         if(cryptoDevTokenReserve == 0) {
             ...
             */
             ...
             'Ether' and determine according to the ratio how many 'CryptoDev' tokens
    ...
    ///@dev Returns the amount Eth/Crypto Dev tokens that would be returned to the userin the swap
    function removeLiquidity(uint _amount) public returns (uint, uint) {
        ...
        // Burn the sent'LP' tokens from the user's a wallet because they are already sent to remove liquidity
        _burn(msg.sender, _amount);
        // Transfer 'ethAmount' of Eth from user's wallet to the contract
        payable(msg.sender).transfer(ethAmount);
        // Transfer 'cryptoDevTokenAmount' of 'CryptoDev' tokens from the user's wallet to the contract
    ...
    */
    function getAmountOfTokens(
        ...
        // We need to make sure (x+Ax)*(y-Ay)=(x)*(y)
        // so the final formula is Ay=(y*Ax)/(x+Ax);
        // Ay in our case is 'tokens to be recieved'
        // Ax=((input amount)*99)/100, x=inputReserve, y=outputReserve
